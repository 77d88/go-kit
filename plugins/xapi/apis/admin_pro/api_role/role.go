...
func save(c *xhs.Ctx) {
	...
	c.Fatalf(xdb.Ctx(c).Model(&pro.Role{}).WithId(r.Id).Updates(map[string]interface{}{
		"name":        r.Name,
...
func del(c *xhs.Ctx) {
	...
	var role pro.Permission
	c.Fatalf(xdb.Ctx(c).WithId(r.Id).First(&role))
	var count int64
	c.Fatalf(xdb.Ctx(c).Model(&pro.Role{}).Where("deleted_time is null and roles && ?", xdb.NewInt8Array(r.Id)).Count(&count))
	c.Fatalf(count > 0, "角色已分配给用户，不能删除")
	// 删除角色
	c.Fatalf(xdb.Ctx(c).Model(&pro.Role{}).WithId(r.Id).
...
func list(c *xhs.Ctx) {
	...
	c.SendPage(xarray.Map(list, func(index int, item pro.Role) *pro.RoleDst {
