...
func ConvertMenusToRouter(allMenus []*pro.Menu, roles ...string) []*Router {
	menuMap := make(map[int64]*pro.Menu)
...
func buildRouterTree(menu *pro.Menu, menuMap map[int64]*pro.Menu, roles []string) *Router {
...
func hasParent(menuID int64, allMenus []*pro.Menu) bool {
	for _, menu := range allMenus {
		for _, childID := range menu.Children.ToSlice() {
			if childID == menuID {
				return true
...
func hasPermission(menu *pro.Menu, userRoles []string) bool {
	if menu.Permission.IsEmpty() {
		return true
	}
	if xarray.Contain(userRoles, pro.RoleSuperAdmin) {
		return true
	}
