...
func saveMenu(c *xhs.Ctx) {
	...
	c.Fatalf(xdb.Ctx(c).SaveMap(&pro.Menu{}, req, map[string]interface{}{
...
func getUserPermissionMenu(c *xhs.Ctx) {
	...
	var user pro.User
	if userId <= 0 {
		c.Fatalf(!c.HasRole(pro.RoleSuperAdmin), "请登录", xhs.FatalWithCode(xhs.CodeTokenError))
	}
...
func setPermission(c *xhs.Ctx) {
	...
	c.Fatalf(xdb.Ctx(c).Model(&pro.Permission{}).WithId(r.PermissionIds.ToSlice()...).Find(&permissions))
	codes := make([]string, 0, len(permissions))
	for _, p := range permissions {
		codes = append(codes, p.Code)
	}
	c.Fatalf(xdb.Ctx(c).Model(&pro.Menu{}).WithId(r.Id).
...
func Register(api *xe.Engine, path string) {
	api.RegisterPost(path+"/all", pro.SuperAdmin, allMenu)                   // 所有菜单
	api.RegisterPost(path+"/save", pro.SuperAdmin, saveMenu)                 // 保存菜单
	api.RegisterPost(path+"/delete", pro.SuperAdmin, deleteMenu)             // 删除菜单
