...
func list(c *xhs.Ctx) {
	...
	c.Fatalf(xdb.Ctx(c).Model(&pro2.User{}).
	...
	c.SendPage(pro2.ToUserResponses(users), total)
}
...
func disable(c *xhs.Ctx) {
	...
	c.Fatalf(xdb.Ctx(c).Model(&pro2.User{}).WithId(r.Id).
...
func save(c *xhs.Ctx) {
	...
	c.Fatalf(xdb.Ctx(c).SaveMap(&pro2.User{}, r, map[string]interface{}{
		"update_user": c.GetUserIdAssert(),
	}))
}
...
func setRoles(c *xhs.Ctx) {
	...
	if !r.Roles.IsEmpty() { // 获取角色对应的权限码
		...
		var roles []pro2.Role
		c.Fatalf(xdb.Ctx(c).Model(&pro2.Role{}).WithId(r.Roles.ToSlice()...).Find(&roles))
		...
		if len(permissionIds) > 0 {
			...
			c.Fatalf(xdb.Ctx(c).Model(&pro2.Permission{}).WithId(permissionIds...).Find(&permission))
...
func info(c *xhs.Ctx) {
	...
	c.Fatalf(r.Id <= 0, "请选择用户")
	var user pro2.User
	c.Fatalf(xdb.Ctx(c).Model(&pro2.User{}).WithId(r.Id).Find(&user))
...
func tokenInfo(c *xhs.Ctx) {
	...
	var user pro2.User
	c.Fatalf(xdb.Ctx(c).Model(&pro2.User{}).WithId(userId).Find(&user))
	c.Send(user.ToResponse())
}
